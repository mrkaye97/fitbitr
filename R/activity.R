...
#'
#' @export
get_activity <- function(token, resource_path, date = "", period = "", base_date = "", end_date = "", simplify = TRUE) {
  url <- if (date != "" && period != "") {
...
#'
#' @export
get_activity_intraday <- function(token, resource_path, date, detail_level = "15min", start_time = NULL, end_time = NULL, simplify = TRUE) {
...
#' @importFrom rlang set_names eval_tidy sym
...
update_activity_goals <- function(token, period, calories_out = NULL, active_minutes = NULL, floors = NULL, distance = NULL, steps = NULL) {
  ...
  body <- map(set_names(post_arguments), ~ eval_tidy(sym(.x)))
  names(body) <- str_replace_all(names(body), "(_)([a-z])", function(x) {
    str_to_upper(str_sub(x, 2))
  })
